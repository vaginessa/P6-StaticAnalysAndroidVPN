...
class FloatingActionButtonIcs extends FloatingActionButtonEclairMr1 {
    ...
    void show(@Nullable final InternalVisibilityChangedListener listener, final boolean fromUser) {
        ...
        if (!ViewCompat.isLaidOut(this.mView) || this.mView.isInEditMode()) {
            ...
            this.mView.setAlpha(PhotoViewAttacher.DEFAULT_MIN_SCALE);
            this.mView.setScaleY(PhotoViewAttacher.DEFAULT_MIN_SCALE);
            this.mView.setScaleX(PhotoViewAttacher.DEFAULT_MIN_SCALE);
            if (listener != null) {
                listener.onShown();
                return;
            }
        ...
        this.mView.animate().scaleX(PhotoViewAttacher.DEFAULT_MIN_SCALE).scaleY(PhotoViewAttacher.DEFAULT_MIN_SCALE).alpha(PhotoViewAttacher.DEFAULT_MIN_SCALE).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new AnimatorListenerAdapter() {
            public void onAnimationStart(Animator animation) {
                FloatingActionButtonIcs.this.mView.internalSetVisibility(0, fromUser);
            }
