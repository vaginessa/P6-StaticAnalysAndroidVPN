...
class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    public static float calculateVerticalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) (SHADOW_MULTIPLIER * maxShadowSize)) + ((WeightedLatLng.DEFAULT_INTENSITY - COS_45) * ((double) cornerRadius)));
        }
    ...
    public static float calculateHorizontalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) maxShadowSize) + ((WeightedLatLng.DEFAULT_INTENSITY - COS_45) * ((double) cornerRadius)));
        }
    ...
    private void drawShadow(Canvas canvas) {
        ...
        boolean drawHorizontalEdges = this.mContentBounds.width() - (PhotoViewAttacher.DEFAULT_MAX_SCALE * shadowOffset) > 0.0f;
        boolean drawVerticalEdges = this.mContentBounds.height() - (PhotoViewAttacher.DEFAULT_MAX_SCALE * shadowOffset) > 0.0f;
        ...
        if (drawHorizontalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (PhotoViewAttacher.DEFAULT_MAX_SCALE * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        if (drawHorizontalEdges) {
            canvas.scale(1.0f / shadowScaleHorizontal, 1.0f);
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (PhotoViewAttacher.DEFAULT_MAX_SCALE * shadowOffset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
        ...
        if (drawVerticalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.height() - (PhotoViewAttacher.DEFAULT_MAX_SCALE * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
    ...
    private void buildShadowCorners() {
        ...
        if (shadowRadius > 0.0f) {
            ...
            float midRatio = startRatio + ((1.0f - startRatio) / PhotoViewAttacher.DEFAULT_MAX_SCALE);
    ...
    public float getMinWidth() {
        return (this.mRawMaxShadowSize * PhotoViewAttacher.DEFAULT_MAX_SCALE) + (PhotoViewAttacher.DEFAULT_MAX_SCALE * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + (this.mRawMaxShadowSize / PhotoViewAttacher.DEFAULT_MAX_SCALE)));
    }

    public float getMinHeight() {
        return ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) * PhotoViewAttacher.DEFAULT_MAX_SCALE) + (PhotoViewAttacher.DEFAULT_MAX_SCALE * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) / PhotoViewAttacher.DEFAULT_MAX_SCALE)));
    }
}
