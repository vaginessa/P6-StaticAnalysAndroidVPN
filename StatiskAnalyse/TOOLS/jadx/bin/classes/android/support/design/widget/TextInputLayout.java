...
public class TextInputLayout extends LinearLayout {
    ...
    private void addIndicator(TextView indicator, int index) {
        if (this.mIndicatorArea == null) {
            ...
            this.mIndicatorArea.addView(new Space(getContext()), new LinearLayout.LayoutParams(0, 0, PhotoViewAttacher.DEFAULT_MIN_SCALE));
            if (this.mEditText != null) {
                adjustIndicatorPadding();
            }
        }
    ...
    public void setError(@Nullable final CharSequence error) {
        if (!TextUtils.equals(this.mError, error)) {
            ...
            if (this.mErrorShown) {
                ...
                if (animate) {
                    if (ViewCompat.getAlpha(this.mErrorView) == PhotoViewAttacher.DEFAULT_MIN_SCALE) {
                        ViewCompat.setAlpha(this.mErrorView, 0.0f);
                    }
                    ViewCompat.animate(this.mErrorView).alpha(PhotoViewAttacher.DEFAULT_MIN_SCALE).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new ViewPropertyAnimatorListenerAdapter() {
    ...
    private void collapseHint(boolean animate) {
        ...
        if (animate && this.mHintAnimationEnabled) {
            animateToExpansionFraction(PhotoViewAttacher.DEFAULT_MIN_SCALE);
        } else {
            this.mCollapsingTextHelper.setExpansionFraction(PhotoViewAttacher.DEFAULT_MIN_SCALE);
        }
    }
