...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
    ...
    public static abstract class ViewHolder {
        ...
        void clearPayload() {
            ...
            this.mFlags &= EMError.ILLEGAL_USER_NAME;
        }
        ...
        public String toString() {
            ...
            if (!isRecyclable()) {
                sb.append(" not recyclable(" + this.mIsRecyclableCount + Separators.RPAREN);
            }
    ...
    public final class Recycler {
        ...
        View getViewForPosition(int position, boolean dryRun) {
            if (position < 0 || position >= RecyclerView.this.mState.getItemCount()) {
                throw new IndexOutOfBoundsException("Invalid item position " + position + Separators.LPAREN + position + "). Item count:" + RecyclerView.this.mState.getItemCount());
            }
            int offsetPosition;
            LayoutParams rvLayoutParams;
        ...
        ViewHolder getScrapViewForPosition(int position, int type, boolean dryRun) {
            ...
            while (i < scrapCount) {
                ...
                if (holder.wasReturnedFromScrap() || holder.getLayoutPosition() != position || holder.isInvalid() || (!RecyclerView.this.mState.mInPreLayout && holder.isRemoved())) {
                    ...
                    Log.e(RecyclerView.TAG, "Scrap view for position " + position + " isn't dirty but has" + " wrong view type! (found " + holder.getItemViewType() + " but expected " + type + Separators.RPAREN);
                    if (!dryRun) {
    ...
    private class ViewFlinger implements Runnable {
        ...
        private int computeScrollDuration(int dx, int dy, int vx, int vy) {
            ...
            float distance = ((float) halfContainerSize) + (((float) halfContainerSize) * distanceInfluenceForSnapDuration(Math.min(PhotoViewAttacher.DEFAULT_MIN_SCALE, (PhotoViewAttacher.DEFAULT_MIN_SCALE * ((float) delta)) / ((float) containerSize))));
            if (velocity > 0) {
                ...
                duration = (int) (((((float) absDx) / ((float) containerSize)) + PhotoViewAttacher.DEFAULT_MIN_SCALE) * 300.0f);
            }
    ...
    private String getFullClassName(Context context, String className) {
        ...
        return !className.contains(Separators.DOT) ? RecyclerView.class.getPackage().getName() + '.' + className : className;
    }
    ...
    private void pullGlows(float x, float overscrollX, float y, float overscrollY) {
        ...
        if (overscrollX < 0.0f) {
            ensureLeftGlow();
            if (this.mLeftGlow.onPull((-overscrollX) / ((float) getWidth()), PhotoViewAttacher.DEFAULT_MIN_SCALE - (y / ((float) getHeight())))) {
                invalidate = true;
            }
        ...
        if (overscrollY < 0.0f) {
            ...
            ensureBottomGlow();
            if (this.mBottomGlow.onPull(overscrollY / ((float) getHeight()), PhotoViewAttacher.DEFAULT_MIN_SCALE - (x / ((float) getWidth())))) {
                invalidate = true;
            }
        }
