...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    private class RecoverAnimation implements AnimatorListenerCompat {
        ...
        public void onAnimationCancel(ValueAnimatorCompat animation) {
            setFraction(PhotoViewAttacher.DEFAULT_MIN_SCALE);
        }
    ...
    public static abstract class Callback {
        ...
        public int interpolateOutOfBoundsScroll(RecyclerView recyclerView, int viewSize, int viewSizeOutOfBounds, int totalSize, long msSinceStartScroll) {
            float timeRatio;
            int cappedScroll = (int) (((float) (((int) Math.signum((float) viewSizeOutOfBounds)) * getMaxDragScroll(recyclerView))) * sDragViewScrollCapInterpolator.getInterpolation(Math.min(PhotoViewAttacher.DEFAULT_MIN_SCALE, (PhotoViewAttacher.DEFAULT_MIN_SCALE * ((float) Math.abs(viewSizeOutOfBounds))) / ((float) viewSize))));
            if (msSinceStartScroll > DRAG_SCROLL_ACCELERATION_LIMIT_TIME_MS) {
                timeRatio = PhotoViewAttacher.DEFAULT_MIN_SCALE;
            } else {
