...
class PathParser {
    ...
    public static class PathDataNode {
        ...
        public void interpolatePathDataNode(PathDataNode nodeFrom, PathDataNode nodeTo, float fraction) {
            for (int i = 0; i < nodeFrom.params.length; i++) {
                this.params[i] = (nodeFrom.params[i] * (PhotoViewAttacher.DEFAULT_MIN_SCALE - fraction)) + (nodeTo.params[i] * fraction);
            }
        }
                ...
                switch (cmd) {
                    ...
                    case 'S':
                        ...
                        if (previousCmd == 'c' || previousCmd == 's' || previousCmd == 'C' || previousCmd == 'S') {
                            reflectiveCtrlPointX = (PhotoViewAttacher.DEFAULT_MAX_SCALE * currentX) - ctrlPointX;
                            reflectiveCtrlPointY = (PhotoViewAttacher.DEFAULT_MAX_SCALE * currentY) - ctrlPointY;
                        }
    ...
    private static float[] getFloats(String s) {
        ...
        try {
            ...
            throw new RuntimeException("error in parsing \"" + s + Separators.DOUBLE_QUOTE, e);
        }
    }
