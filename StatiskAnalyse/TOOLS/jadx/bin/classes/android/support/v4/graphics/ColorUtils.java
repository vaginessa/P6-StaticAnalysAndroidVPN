...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        float l = (max + min) / PhotoViewAttacher.DEFAULT_MAX_SCALE;
        if (max == min) {
            s = 0.0f;
            h = 0.0f;
        } else {
            if (max == rf) {
                ...
                h = ((bf - rf) / deltaMaxMin) + PhotoViewAttacher.DEFAULT_MAX_SCALE;
            } else {
                h = ((rf - gf) / deltaMaxMin) + 4.0f;
            }
            s = deltaMaxMin / (PhotoViewAttacher.DEFAULT_MIN_SCALE - Math.abs((PhotoViewAttacher.DEFAULT_MAX_SCALE * l) - PhotoViewAttacher.DEFAULT_MIN_SCALE));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[1] = constrain(s, 0.0f, (float) PhotoViewAttacher.DEFAULT_MIN_SCALE);
        outHsl[2] = constrain(l, 0.0f, (float) PhotoViewAttacher.DEFAULT_MIN_SCALE);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float s = hsl[1];
        float l = hsl[2];
        float c = (PhotoViewAttacher.DEFAULT_MIN_SCALE - Math.abs((PhotoViewAttacher.DEFAULT_MAX_SCALE * l) - PhotoViewAttacher.DEFAULT_MIN_SCALE)) * s;
        float m = l - (0.5f * c);
        float x = c * (PhotoViewAttacher.DEFAULT_MIN_SCALE - Math.abs(((h / 60.0f) % PhotoViewAttacher.DEFAULT_MAX_SCALE) - PhotoViewAttacher.DEFAULT_MIN_SCALE));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
