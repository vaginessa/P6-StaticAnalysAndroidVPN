...
public class ActionBarDrawerToggle implements DrawerListener {
    ...
    public void syncState() {
        if (this.mDrawerLayout.isDrawerOpen((int) GravityCompat.START)) {
            this.mSlider.setPosition(PhotoViewAttacher.DEFAULT_MIN_SCALE);
        } else {
    ...
    public void onDrawerSlide(View drawerView, float slideOffset) {
        ...
        if (slideOffset > 0.5f) {
            glyphOffset = Math.max(glyphOffset, Math.max(0.0f, slideOffset - 0.5f) * PhotoViewAttacher.DEFAULT_MAX_SCALE);
        } else {
            glyphOffset = Math.min(glyphOffset, slideOffset * PhotoViewAttacher.DEFAULT_MAX_SCALE);
        }
