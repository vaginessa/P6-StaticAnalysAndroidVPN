...
public class ViewDragHelper {
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (PhotoViewAttacher.DEFAULT_MIN_SCALE / sensitivity));
        return helper;
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(PhotoViewAttacher.DEFAULT_MIN_SCALE, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + PhotoViewAttacher.DEFAULT_MIN_SCALE) * 256.0f);
        }
