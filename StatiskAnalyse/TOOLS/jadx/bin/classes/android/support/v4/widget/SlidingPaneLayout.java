...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? PhotoViewAttacher.DEFAULT_MIN_SCALE : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
                if (isLayoutRtl) {
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(PhotoViewAttacher.DEFAULT_MIN_SCALE, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == PhotoViewAttacher.DEFAULT_MIN_SCALE;
    }
    ...
    private void parallaxOtherViews(float slideOffset) {
            ...
            if ((isLayoutRtl ? slideLp.rightMargin : slideLp.leftMargin) <= 0) {
                ...
                for (i = 0; i < childCount; i++) {
                    ...
                    if (v == this.mSlideableView) {
                        int oldOffset = (int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE - this.mParallaxOffset) * ((float) this.mParallaxBy));
                        this.mParallaxOffset = slideOffset;
                        dx = oldOffset - ((int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE - slideOffset) * ((float) this.mParallaxBy)));
                        if (isLayoutRtl) {
                            dx = -dx;
                        }
                        v.offsetLeftAndRight(dx);
                        if (!dimViews) {
                            dimChildView(v, isLayoutRtl ? this.mParallaxOffset - PhotoViewAttacher.DEFAULT_MIN_SCALE : PhotoViewAttacher.DEFAULT_MIN_SCALE - this.mParallaxOffset, this.mCoveredFadeColor);
                        }
                    }
                }
            }
        ...
        for (i = 0; i < childCount; i++) {
            v = getChildAt(i);
            if (v == this.mSlideableView) {
                int oldOffset2 = (int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                dx = oldOffset2 - ((int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE - slideOffset) * ((float) this.mParallaxBy)));
                if (isLayoutRtl) {
                    dx = -dx;
                }
                v.offsetLeftAndRight(dx);
