...
class PathInterpolatorDonut implements Interpolator {
    ...
    public float getInterpolation(float t) {
        ...
        if (t >= PhotoViewAttacher.DEFAULT_MIN_SCALE) {
    ...
    private static Path createQuad(float controlX, float controlY) {
        ...
        path.quadTo(controlX, controlY, PhotoViewAttacher.DEFAULT_MIN_SCALE, PhotoViewAttacher.DEFAULT_MIN_SCALE);
        return path;
    }
    ...
    private static Path createCubic(float controlX1, float controlY1, float controlX2, float controlY2) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
        path.cubicTo(controlX1, controlY1, controlX2, controlY2, PhotoViewAttacher.DEFAULT_MIN_SCALE, PhotoViewAttacher.DEFAULT_MIN_SCALE);
        return path;
    }
}
