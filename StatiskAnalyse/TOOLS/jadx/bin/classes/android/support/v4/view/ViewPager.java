...
public class ViewPager extends ViewGroup {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        public float getInterpolation(float t) {
            t -= PhotoViewAttacher.DEFAULT_MIN_SCALE;
            return ((((t * t) * t) * t) * t) + PhotoViewAttacher.DEFAULT_MIN_SCALE;
        }
    };
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (PhotoViewAttacher.DEFAULT_MAX_SCALE * density);
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(PhotoViewAttacher.DEFAULT_MIN_SCALE, (PhotoViewAttacher.DEFAULT_MIN_SCALE * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((PhotoViewAttacher.DEFAULT_MIN_SCALE + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
            ...
            while (curIndex < this.mItems.size()) {
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        ...
                        leftWidthNeeded = clientWidth > 0 ? 0.0f : (PhotoViewAttacher.DEFAULT_MAX_SCALE - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth));
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        itemIndex = curIndex + 1;
                        if (extraWidthRight < PhotoViewAttacher.DEFAULT_MAX_SCALE) {
                            ii = itemIndex >= this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex) : null;
                            rightWidthNeeded = clientWidth > 0 ? 0.0f : (((float) getPaddingRight()) / ((float) clientWidth)) + PhotoViewAttacher.DEFAULT_MAX_SCALE;
                            pos = this.mCurItem + 1;
                            while (pos < N) {
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - PhotoViewAttacher.DEFAULT_MIN_SCALE : Float.MAX_VALUE;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - PhotoViewAttacher.DEFAULT_MIN_SCALE;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + PhotoViewAttacher.DEFAULT_MIN_SCALE)) * ((float) width));
