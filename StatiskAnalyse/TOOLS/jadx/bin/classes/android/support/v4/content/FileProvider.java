...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(Separators.SLASH)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, Separators.SLASH)).build();
            } catch (IOException e) {
    ...
    private static int modeToMode(String mode) {
        ...
        if (Config.DEVICE_WIDTH.equals(mode) || "wt".equals(mode)) {
            return 738197504;
        }
        if ("wa".equals(mode)) {
            return 704643072;
