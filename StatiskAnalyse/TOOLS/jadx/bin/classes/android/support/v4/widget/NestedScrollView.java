                        ...
                        if (!dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY - scrolledDeltaY, this.mScrollOffset)) {
                            if (canOverscroll) {
                                ...
                                if (pulledToY < 0) {
                                    ...
                                    this.mEdgeGlowBottom.onPull(((float) deltaY) / ((float) getHeight()), PhotoViewAttacher.DEFAULT_MIN_SCALE - (MotionEventCompat.getX(ev, activePointerIndex) / ((float) getWidth())));
