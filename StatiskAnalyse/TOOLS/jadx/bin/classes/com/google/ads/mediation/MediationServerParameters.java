    ...
    public void load(Map<String, String> parameters) throws MappingException {
        ...
        for (Entry entry : parameters.entrySet()) {
            ...
            if (field2 != null) {
                ...
                zzb.zzaF("Unexpected server option: " + ((String) entry.getKey()) + " = \"" + ((String) entry.getValue()) + Separators.DOUBLE_QUOTE);
            }
        }
        StringBuilder stringBuilder = new StringBuilder();
        for (Field field22 : hashMap.values()) {
            if (((Parameter) field22.getAnnotation(Parameter.class)).required()) {
                zzb.zzaH("Required server option missing: " + ((Parameter) field22.getAnnotation(Parameter.class)).name());
